...
import { getMyTickets, getCampaign, getCampaigns } from '../api';
...
const BADGES = [
  ...
  { id: 'early_bird', icon: '⏰', title: 'Early Bird', desc: 'Enter a campaign in the first hour', condition: (_s: any, t: any[]) => t.some((tkt: any) => tkt.early_bird) },
];
...
function NotificationBell({ notifications }: { notifications: { id: string; text: string; type: string; time: string; read: boolean }[] }) {
    ...
    <div className="relative">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="px-4 py-3 border-b border-gray-100 bg-light-gray">
            ...
            {notifications.length > 0 && (
              <button className="w-full py-2.5 text-center text-gold font-bold text-xs hover:bg-light-gray transition-colors">
...
export default function DashboardContent() {
  ...
  useEffect(() => {
    ...
    updateStat('streak', parseInt(String(streak)));
  }, []);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const allC = await getCampaigns();
      setAllCampaigns(Array.isArray(allC) ? allC : []);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    const ending = allCampaigns.filter(c => {
      const diff = new Date(c.draw_date).getTime() - Date.now();
      return diff > 0 && diff < 4 * 60 * 60 * 1000;
    });
    ending.forEach(c => {
      const diff = Math.round((new Date(c.draw_date).getTime() - Date.now()) / 3600000);
      const existing = notifications.find(n => n.text.includes(c.title));
      if (!existing) {
        setNotifications(prev => [{
          id: `ending_${c.id}`,
          text: `⏰ ${c.title} ends in ${diff}h — enter now before it's too late!`,
          type: 'ending',
          time: 'Just now',
          read: false,
        }, ...prev]);
  ...
  const playChaChing = () => {
    ...
    try { const a = new Audio('data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YU'; a.volume = 0.3; a.play().catch(() => {}); } catch {}
  };

  const shareToX = (campaign: any) => {
    const text = `I just entered to win ${campaign.title} on @WINBIGAFRICA! 🎉🏆 #WINBIG #Nigeria`;
    window.open(`https://x.com/intent/tweet?text=${encodeURIComponent(text)}`, '_blank');
  };

  const shareToWhatsApp = (campaign: any) => {
    const text = `I just entered to win ${campaign.title} on WINBIG AFRICA! 🎉🏆\n\nJoin me here: https://winbig.africa/campaigns`;
  ...
  const lostTickets = tickets.filter((t: any) => t.status !== 'won');
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-start justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              {notifications.some(n => !n.read) && (
                <button onClick={markAllRead} className="text-xs text-gold font-bold hover:underline">Mark all read</button>
              )}
            </div>
          </motion.div>
              ...
              <motion.div key="tickets" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                {loading ? (
                  ...
                  <>
                    {wonTickets.length > 0 && (
                      <div className="mb-6">
                        <h3 className="font-black text-deep-blue mb-3 flex items-center gap-2">
                          <span className="text-gold">🏆</span> My Wins ({wonTickets.length})
                        </h3>
                          ...
                          {wonTickets.map((ticket: any, i: number) => (
                            ...
                            >
                              ...
                              <div className="flex gap-1">
                                <button onClick={() => shareToX(campaigns[ticket.campaign_id] || { title: 'WINBIG Prize' })} className="p-2 rounded-lg bg-white border border-gray-100 hover:bg-gold/10 transition-colors" title="Share on X">
                                ...
                                <button onClick={() => shareToWhatsApp(campaigns[ticket.campaign_id] || { title: 'WINBIG Prize' })} className="p-2 rounded-lg bg-white border border-gray-100 hover:bg-green-50 transition-colors" title="Share on WhatsApp">
                    ...
                    <div>
                      <h3 className="font-black text-deep-blue mb-3 flex items-center gap-2">
                        <span>🎟️</span> All Tickets ({tickets.length})
                      </h3>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {ticket.status === 'pending' && (
                                <button
                                  onClick={() => shareToX(campaigns[ticket.campaign_id] || { title: 'WINBIG Campaign' })}
              ...
              <motion.div key="referral" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="bg-white rounded-2xl p-8 border border-gray-100 text-center max-w-lg mx-auto">
                  ...
                  <div className="flex justify-center gap-3">
                    <button
                      onClick={() => { const l = `https://winbig.africa/register?ref=${referralCode}`; window.open(`https://x.com/intent/tweet?text=${encodeURIComponent(`Join WINBIG AFRICA and win amazing prizes! Use my code: ${referralCode}\n\nhttps://winbig.africa`)})`, '_blank'); }}
                    ...
                    <button
                      onClick={() => { const l = `https://winbig.africa/register?ref=${referralCode}`; window.open(`https://wa.me/?text=${encodeURIComponent(`Join WINBIG AFRICA and win amazing prizes! Use my code: ${referralCode}\n\n${l}`)})`, '_blank'); }}
                  ...
                  <p className="text-gray-400 text-xs mt-6">Bonus is credited within 24 hours of your friend&apos;s first ticket purchase. No limit on referrals!</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
