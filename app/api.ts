...
// --- Paystack wallet helpers ---
export async function initializePaystack(email: string, amount: number) {
...
export { initializePaystack as initializePaystackPayment };

export async function processPaystackPayment(reference: string, campaignId: string, quantity: number, token: string) {
  const res = await fetch(`${API_BASE}/api/paystack/verify`, {
    ...
    body: JSON.stringify({ reference, campaign_id: campaignId, quantity })
  });
  return res.json();
}
